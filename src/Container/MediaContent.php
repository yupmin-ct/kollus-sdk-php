...
class MediaContent extends AbstractContainer
{
    ...
    public function __construct($items = [])
    {
        ...
        if (isset($items['subtitles']) && is_array($items['subtitles'])) {
            foreach ($items['subtitles'] as $node) {
                $this->subtitles->appendElement(new Subtitle($node));
            }
            unset($items['subtitles']);
        }
        ...
        if (isset($items['transcoding_files'])) {
            foreach ($items['transcoding_files'] as $transcoding_file) {
                $this->transcoding_files->appendElement(new TranscodingFile($transcoding_file));
            }
        }
